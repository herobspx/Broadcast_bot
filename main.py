    ...
    try:
        async with httpx.AsyncClient(timeout=10) as c:
            ...
            if r.status_code == 200:
                data = r.json()
                if data:
                    if "last_closed" in data:
                        return data["last_closed"]
                    trade = list(data.values())[0]
                    return trade

            r2 = await c.get(f"{SIGNALS_URL}/closed_trades")
...
def make_combined_image(trade: dict) -> io.BytesIO:
    ...
    contract_data = {
        ...
        "bid":           trade.get("bid", 0),
        "ask":           trade.get("ask", 0),
        "mid":           trade.get("mid", entry),
        "open":          trade.get("open", entry),
        "high":          trade.get("high", entry),
        "low":           trade.get("low", entry),
        "volume":        trade.get("volume", trade.get("vol", "--")),
        "open_interest": trade.get("oi", "--"),
    }

    path1 = generate_trade_card(contract_data, current_price=entry,     status="OPEN")
    path2 = generate_trade_card(contract_data, current_price=max_price, status="HIGH")
...
def build_caption(trade: dict) -> str:
    entry     = float(trade.get("entry", 0))
    max_price = float(trade.get("max_price", entry))
    pnl_usd   = (max_price - entry) * 100
    pnl_sar   = pnl_usd * 3.75
    ...
    return (
        ...
        f"💰 الربـح : {pnl_usd:.0f}$ ({pnl_sar:.0f} ريال) 🔥\n"
        f"ـــــــ\n"
        f"🎁 انضم الآن مجانًا لمدة 48 ساعة 🕰️\n"
        f"[👉 انضم الآن](https://{BOT_LINK})\n"
        f"ـــــــ\n"
        f"⚠️ العدد محدود في البداية"
    )

def build_tiktok_caption(trade: dict) -> str:
    entry     = float(trade.get("entry", 0))
    max_price = float(trade.get("max_price", entry))
    pnl_usd   = (max_price - entry) * 100
    pnl_pct   = ((max_price - entry) / entry * 100) if entry else 0
    pnl_sar   = pnl_usd * 3.75
    symbol    = trade.get("symbol", "SPXW")
    strike    = trade.get("strike", "")
    opt_type  = trade.get("type", "CALL").upper()

    return (
        f"🔥 صفقة اليوم — {symbol} {strike} {opt_type}\n\n"
        f"📥 دخول: ${entry:.2f}\n"
        f"📤 خروج: ${max_price:.2f}\n"
        f"💰 +{pnl_usd:.0f}$ ({pnl_sar:.0f} ريال) | +{pnl_pct:.1f}%\n\n"
        f"📊 إشارات يومية مجانية 👇\n"
        f"t.me/bam_spx\n\n"
        f"#SPX #0DTE #تداول #اسهم #SPXW #trading #options"
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "trade_confirm":
        ...
        try:
            ...
            # ── إرسال نسخة TikTok للأدمن ──
            tiktok_caption = build_tiktok_caption(trade)
            combined2 = make_combined_image(trade)
            await context.bot.send_photo(
                chat_id=query.message.chat_id,
                photo=combined2,
                caption=f"📱 *نسخة TikTok — احفظها وانشرها*\n\n`{tiktok_caption}`",
                parse_mode="Markdown"
            )
